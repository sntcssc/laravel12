<div class="container mx-auto p-4">
    <div class="flex justify-between items-center mb-4">
        ...
        <a href="{{ route('sections.create') }}" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">Add Section</a>
    </div>

    <!-- Search and Filter -->
    <div class="mb-4">
        <input wire:model.live.debounce.500ms="search" type="text" placeholder="Search sections..." class="w-full p-2 border rounded dark:bg-gray-800 dark:text-white dark:border-gray-600">
    </div>
    ...
    <table class="min-w-full bg-white dark:bg-gray-800 border dark:border-gray-700">
        <thead>
            <tr>
                <th class="p-2"><input type="checkbox" wire:model="selectedSections" value=""></th>
        ...
        <tbody>
            @foreach ($sections as $section)
                <tr class="{{ $section->deleted_at ? 'bg-gray-200 dark:bg-gray-700' : '' }}">
                    <td class="p-2"><input type="checkbox" wire:model="selectedSections" value="{{ $section->id }}"></td>
                    <td class="p-2">{{ $section->name }}</td>
                    <td class="p-2">{{ $section->programme ? $section->programme->name : 'N/A' }}</td>
                    <td class="p-2">
                        @if ($section->deleted_at)
                            <button wire:click="restore({{ $section->id }})" class="text-green-500 hover:text-green-600">Restore</button>
                        @else
                            <a href="{{ route('sections.edit', $section->id) }}" class="text-blue-500 hover:text-blue-600">Edit</a>
                            <button wire:click="delete({{ $section->id }})" class="text-red-500 hover:text-red-600" onclick="return confirm('Are you sure?')">Delete</button>
                        @endif
                    </td>
                </tr>
            @endforeach
