<div class="container mx-auto p-4">
    <div class="flex justify-between items-center mb-4">
        ...
        <a href="{{ route('attendances.create') }}" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">Add Attendance</a>
    </div>

    <!-- Search and Filter -->
    <div class="mb-4">
        <input wire:model.live.debounce.500ms="search" type="text" placeholder="Search attendances..." class="w-full p-2 border rounded dark:bg-gray-800 dark:text-white dark:border-gray-600">
    </div>
    ...
    <table class="min-w-full bg-white dark:bg-gray-800 border dark:border-gray-700">
        <thead>
            <tr>
                <th class="p-2"><input type="checkbox" wire:model="selectedAttendances" value=""></th>
                ...
                <th wire:click="sortBy('date')" class="cursor-pointer p-2">Date @if($sortField === 'date') {{ $sortDirection === 'asc' ? '↑' : '↓' }} @endif</th>
        ...
        <tbody>
            @foreach ($attendances as $attendance)
                <tr class="{{ $attendance->deleted_at ? 'bg-gray-200 dark:bg-gray-700' : '' }}">
                    <td class="p-2"><input type="checkbox" wire:model="selectedAttendances" value="{{ $attendance->id }}"></td>
                    <td class="p-2">{{ $attendance->student->name }}</td>
                    <td class="p-2">{{ $attendance->programme->name }}</td>
                    <td class="p-2">{{ $attendance->batch->name }}</td>
                    <td class="p-2">{{ \Carbon\Carbon::parse($attendance->date)->format('Y-m-d') }}</td>
                    <td class="p-2">{{ ucfirst($attendance->status) }}</td>
                    <td class="p-2">
                        @if ($attendance->deleted_at)
                            <button wire:click="restore({{ $attendance->id }})" class="text-green-500 hover:text-green-600">Restore</button>
                        @else
                            <a href="{{ route('attendances.edit', $attendance->id) }}" class="text-blue-500 hover:text-blue-600">Edit</a>
                            <button wire:click="delete({{ $attendance->id }})" class="text-red-500 hover:text-red-600" onclick="return confirm('Are you sure?')">Delete</button>
                        @endif
                    </td>
                </tr>
            @endforeach
